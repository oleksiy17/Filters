...
{ 
    ...
    reset_states->cd1_ord2[0].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd1_ord2[0].w2 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd1_ord2[1].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd1_ord2[1].w2 = (tStereo_cross) { 0.0, 0.0 };

    reset_states->corr_ord1[0] = (tStereo_cross) { 0.0, 0.0 };
    reset_states->corr_ord1[1] = (tStereo_cross) { 0.0, 0.0 };

    reset_states->corr_ord2[0].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->corr_ord2[0].w2 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->corr_ord2[1].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->corr_ord2[1].w2 = (tStereo_cross) { 0.0, 0.0 };
    ...
    reset_states->cd2_ord2[0].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd2_ord2[0].w2 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd2_ord2[1].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd2_ord2[1].w2 = (tStereo_cross) { 0.0, 0.0 };
    ...
    reset_states->cd3_ord2[0].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd3_ord2[0].w2 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd3_ord2[1].w1 = (tStereo_cross) { 0.0, 0.0 };
    reset_states->cd3_ord2[1].w2 = (tStereo_cross) { 0.0, 0.0 };
    ...
    size_t      samples_count)
{
    ...
    for (my_uint32 i = 0; i < samples_count; i++)
    {
        ...
        pr_states->in.L = ((tStereo_cross*)pr_audio->audio)[i].L*0.5;
        pr_states->in.R = ((tStereo_cross*)pr_audio->audio)[i].R*0.5;
        //Cascade 1 Parallel 1

        y_ord_1 = order_1(&pr_states->in, &pr_coeffs->band[MID_CUTOFF].ord_1, &pr_states->cd1_ord1[PAR_1]);
        y_ord_2 = order_2(&pr_states->in, &pr_coeffs->band[MID_CUTOFF].ord_2, &pr_states->cd1_ord2[PAR_1]);
        acc_ord_2 = y_ord_2;
        ...
        y_ord_2 = order_2(&sum, &pr_coeffs->band[MID_CUTOFF].ord_2, &pr_states->cd1_ord2[PAR_2]);
        ...
        y_ord_1 = order_1(&low_pass, &pr_coeffs->band[HIG_CUTOFF].ord_1, &pr_states->corr_ord1[NORM_LOW]);
        y_ord_2 = order_2( &y_ord_1, &pr_coeffs->band[HIG_CUTOFF].ord_2, &pr_states->corr_ord2[NORM_LOW]);        // error


        // Cascade 2 Parallel 1
        y_ord_2.L = y_ord_2.L*0.5;
        y_ord_2.R = y_ord_2.R*0.5;

        y_ord_1 = order_1(&y_ord_2, &pr_coeffs->band[LOW_CUTOFF].ord_1, &pr_states->cd2_ord1[PAR_1]);
        y_ord_2 = order_2(&y_ord_2, &pr_coeffs->band[LOW_CUTOFF].ord_2, &pr_states->cd2_ord2[PAR_1]);
        acc_ord_2 = y_ord_2;
        ...
        y_ord_2 = order_2(&sum, &pr_coeffs->band[LOW_CUTOFF].ord_2, &pr_states->cd2_ord2[PAR_2]);
        ...
        //((tStereo_cross*)pr_audio->cross_b.band_1)[i] = pr_states->in;                                                ///////////// BAND_1
        //  Cascade 2 Serial
        y_ord_1 = order_1(&acc_ord_2, &pr_coeffs->band[LOW_CUTOFF].ord_1, &pr_states->cd2_ord1[SERIAL]);

        y_ord_1.L = y_ord_1.L * 2.0f;
        y_ord_1.R = y_ord_1.R * 2.0f;
        ...
        y_ord_1 = order_1(&high_pass, &pr_coeffs->band[LOW_CUTOFF].ord_1, &pr_states->corr_ord1[NORM_HIG]);
        y_ord_2 = order_2(&y_ord_1, &pr_coeffs->band[LOW_CUTOFF].ord_2, &pr_states->corr_ord2[NORM_HIG]);        // error
        ...
        y_ord_2 = order_2(&y_ord_2, &pr_coeffs->band[HIG_CUTOFF].ord_2, &pr_states->cd3_ord2[PAR_1]);
        acc_ord_2 = y_ord_2;
        ...
        y_ord_2 = order_2(&sum, &pr_coeffs->band[HIG_CUTOFF].ord_2, &pr_states->cd3_ord2[PAR_2]);
        ...
        ((tStereo_cross*)pr_audio->audio)[i].R = ((tStereo_cross*)pr_audio->cross_b.band_1)[i].R;


        //((tStereo_cross*)pr_audio->audio)[i].L = ((tStereo_cross*)pr_audio->cross_b.band_1)[i].L;
        //((tStereo_cross*)pr_audio->audio)[i].R = ((tStereo_cross*)pr_audio->cross_b.band_1)[i].R;
        //((tStereo_cross*)pr_audio->audio)[i].L = pr_states->in.L;
        //((tStereo_cross*)pr_audio->audio)[i].R = pr_states->in.R;
...
/*inline tStereo_cross order_1(tStereo_cross *in, coef_1_ord *coeffs, tStereo_cross *delay )
{
    float xh;
    tStereo_cross out;
...
inline tStereo_cross order_2(tStereo_cross *in, coef_2_ord *coeffs, tStereo_cross *delay1, tStereo_cross *delay2)
{
    tStereo_cross out;
