    ...
    void*       states)
{
    ...
    reset_states->comp_ch_1_st.g_sPrev = 0.0; //
    ...
    size_t      samples_count)
{
    ...
    {
        xL_abs = abs_f(((tStereo_compr*)audio_c->cross_b.band_1)[i].L);
        xR_abs = abs_f(((tStereo_compr*)audio_c->cross_b.band_1)[i].R);    

        max_abs = (xL_abs > xR_abs) ? xL_abs : xR_abs;

        if (max_abs >= states_4ch_c->comp_ch_1_st.envelope_prev)              // comparison of current gain and previos gain
        {
            max_abs = mul_f(max_abs, coeffs_4ch_c->comp_ch_1_coef._one_env_att);
            max_abs = mac_f(coeffs_4ch_c->comp_ch_1_coef.attackEnv, states_4ch_c->comp_ch_1_st.envelope_prev, max_abs);
            states_4ch_c->comp_ch_1_st.envelope = max_abs;
        }
        else
        {
            max_abs = mul_f(max_abs, coeffs_4ch_c->comp_ch_1_coef._one_env_rel);
            max_abs = mac_f(coeffs_4ch_c->comp_ch_1_coef.releaseEnv, states_4ch_c->comp_ch_1_st.envelope_prev, max_abs);
            states_4ch_c->comp_ch_1_st.envelope = max_abs;
        }

        states_4ch_c->comp_ch_1_st.envelope_prev = states_4ch_c->comp_ch_1_st.envelope;


        if (states_4ch_c->comp_ch_1_st.envelope < coeffs_4ch_c->comp_ch_1_coef.threshold)
        {
            states_4ch_c->comp_ch_1_st.g_c = 1.0;
        }
        else
        {
            max_abs = div_f(coeffs_4ch_c->comp_ch_1_coef.threshold, states_4ch_c->comp_ch_1_st.envelope);
            max_abs = pow_f(max_abs, coeffs_4ch_c->comp_ch_1_coef._rep_ratio);
            states_4ch_c->comp_ch_1_st.g_c = max_abs;
        }


        if (states_4ch_c->comp_ch_1_st.g_c <= states_4ch_c->comp_ch_1_st.g_sPrev)
        {
            max_abs = mul_f(states_4ch_c->comp_ch_1_st.g_c, coeffs_4ch_c->comp_ch_1_coef._one_g_att);
            max_abs = mac_f(coeffs_4ch_c->comp_ch_1_coef.alphaAttack, states_4ch_c->comp_ch_1_st.g_sPrev, max_abs);
            states_4ch_c->comp_ch_1_st.g_s = max_abs;
        }
        else
        {
            max_abs = mul_f(states_4ch_c->comp_ch_1_st.g_c, coeffs_4ch_c->comp_ch_1_coef._one_g_rel);
            max_abs = mac_f(coeffs_4ch_c->comp_ch_1_coef.alphaRelease, states_4ch_c->comp_ch_1_st.g_sPrev, max_abs);
            states_4ch_c->comp_ch_1_st.g_s = max_abs;
        }

        states_4ch_c->comp_ch_1_st.g_sPrev = states_4ch_c->comp_ch_1_st.g_s;
        states_4ch_c->comp_ch_1_st.g_m = mul_f(states_4ch_c->comp_ch_1_st.g_s, coeffs_4ch_c->comp_ch_1_coef.makeUpGain);

        ((tStereo_compr*)audio_c->cross_b.band_1)[i].L = ((tStereo_compr*)audio_c->cross_b.band_1)[i].L*states_4ch_c->comp_ch_1_st.g_s;
        ((tStereo_compr*)audio_c->cross_b.band_1)[i].R = ((tStereo_compr*)audio_c->cross_b.band_1)[i].R*states_4ch_c->comp_ch_1_st.g_s;
    }
        //((tStereo_compr*)audio_c->audio)[i].L = mul_f(((tStereo_compr*)audio_c->audio)[i].L, states_4ch_c->comp_ch_1_st.g_m);
        //((tStereo_compr*)audio_c->audio)[i].R = mul_f(((tStereo_compr*)audio_c->audio)[i].R, states_4ch_c->comp_ch_1_st.g_m);*/
